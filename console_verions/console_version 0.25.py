...
def find_glass_blocks(desired_color, n_desired_blocks):
    ...
    glass_blocks = []
    true_min_dist = 99999999999
    true_best_glass_blocks = []
    for current_index in range(n_desired_blocks - 1, -1, -1):
        ...
        if min_dist < true_min_dist:
            ...
            true_best_glass_blocks.append(best_color)

    glass_block_names = [rgbToColour[tuple(color)] for color in glass_blocks]
    true_best_glass_blocks_names = [rgbToColour[tuple(color)] for color in true_best_glass_blocks]
    return list(reversed(glass_block_names)), min_dist, true_best_glass_blocks_names, true_min_dist

print(find_glass_blocks((55,68,255), 500))